...
/*
 * File: Hal/idt.cc
 * The x86's Interrupt Descriptor Table (IDT) management.
 *
...
static void set_idt_entry(int idx, void *handler, int ist, int dpl)
{
	uintptr_t addr = (uintptr_t) handler;
	idt_table[idx].offset0 = addr;
	idt_table[idx].cs = __KERNEL_CS;
	idt_table[idx].ist = ist;
	idt_table[idx].flags = 0x8e | (dpl << 5);
	idt_table[idx].offset1 = addr >> 16;
	idt_table[idx].offset2 = addr >> 32;
...
static void panic_with_regs(const char *msg, entry_regs *regs)
{
	KePanic("%s\n"
...
extern "C" void handle_GP_exception (entry_regs *regs)
{
	panic_with_regs("General-Protection fault in userspace!", regs);
}

extern "C" void handle_GP_exception_k(entry_regs *regs)
{
...
static void panic_on_page_fault(uintptr_t addr, entry_regs *regs)
{
...
extern "C" void handle_PF_exception(entry_regs *regs)
{
	uintptr_t fault_addr = read_cr2();
	panic_on_page_fault(fault_addr, regs);
}

extern "C" void handle_PF_exception_k(entry_regs *regs)
{
	uintptr_t fault_addr = read_cr2();
	panic_on_page_fault(fault_addr, regs);
}
