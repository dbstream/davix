...
/*
 * File: include/dsl/vmatree.h
 * VMA trees.
 *
 * Copyright (C) 2025  dbstream
 *
 * The VMA tree is an augmented AVL tree which, in addition to keeping track of
 * range starts and ends, also keeps track of extra information which speeds up
 * searching for free areas.
 */
#pragma once
...
struct VMATree {
	 ...
	 */
	inline VMANode *next(VMANode *node)
	{
		ListHead *entry;
	 ...
	 */
	inline VMANode *prev(VMANode *node)
	{
		ListHead *entry;
	 ...
	 */
	VMANode *find_above(uintptr_t addr);
	 ...
	 */
	VMANode *find_below(uintptr_t addr);
	 ...
	 */
	bool find_free_bottomup(uintptr_t *out,
	 ...
	 * Returns false if no suitable free hole was found.
	 */
	bool find_free_topdown(uintptr_t *out,
			uintptr_t size, uintptr_t align,
			uintptr_t min_addr, uintptr_t max_addr);
...
public:
	...
	static inline T *container_of(VMANode *node)
	{
		return ::container_of(F, node);
	}

	static inline T *container_of_or_null(VMANode *node)
	{
	...
	inline bool find_free_topdown(uintptr_t *out,
			uintptr_t size, uintptr_t align,
			uintptr_t min_addr, uintptr_t max_addr)
	{
