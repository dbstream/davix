...
#
# File: acpi/update_uacpi.sh
# A script for updating uACPI.
#
...
uacpi_version_escaped=$(printf '%s\n' "$uacpi_version" | sed -e 's/[\/&]/\\&/g')
sed -i -E -e "s/(Current uACPI version: ).*/\\1$uacpi_version_escaped/" README
mkdir uacpi
