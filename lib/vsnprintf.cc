...
static inline void put_single_char(snprintf_output &o, char c)
{
	if (o.len) {
		*(o.pos++) = c;
		o.len--;
...
static inline void rep_single_char(snprintf_output &o, char c, size_t n)
{
	if (o.len < n)
		n = o.len;
	memset(o.pos, c, n);
...
static inline void put_string(snprintf_output &o, const char *s, size_t n)
{
	if (o.len < n)
		n = o.len;
	memcpy(o.pos, s, n);
...
static inline void put_string_auto(snprintf_output &o, const char *s)
{
...
template<int base>
static void do_unsigned(snprintf_output &o, fmtspec &spec, printf_uintmax_t value)
{
...
static void do_signed(snprintf_output &o, fmtspec &spec, printf_intmax_t value)
{
	char sign = 0;
...
static void do_string(snprintf_output &o, fmtspec &spec, const char *s)
{
	if (!s)
		s = "(nullptr)";
...
static inline bool try_consume(const char *&fmt, char c)
{
	if (*fmt == c) {
		fmt++;
		return true;
...
static inline int dumb_atoi(const char *&fmt)
{
	int x = *(fmt++) - '0';
	for (int i = 0; i < 4; i++) {
		if (*fmt < '0' || '9' < *fmt)
	...
	for (;;) {
		...
		case 'i':
			if (size == 0 || size == 1 || size == 2)
				...
				do_signed(o, spec, va_arg(args, printf_intmax_t));
			else if (size == 6)
		...
		case 'u':
			if (size == 0 || size == 1 || size == 2)
				...
				do_unsigned<10>(o, spec, va_arg(args, printf_uintmax_t));
			else if (size == 6)
		...
		case 'o':
			if (size == 0 || size == 1 || size == 2)
				...
				do_unsigned<8>(o, spec, va_arg(args, printf_uintmax_t));
			else if (size == 6)
		...
		case 'x':
			if (size == 0 || size == 1 || size == 2)
				...
				do_unsigned<16>(o, spec, va_arg(args, printf_uintmax_t));
			else if (size == 6)
		...
		case 'p':
			...
			do_unsigned<16>(o, spec, (unsigned long) (va_arg(args, void *)));
			break;
		default:
			put_string_auto(o, "(bad format string)");
			*o.pos = 0;
